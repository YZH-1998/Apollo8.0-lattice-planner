...
void ComputeInitFrenetState(
    ...
    const TrajectoryPoint& cartesian_state,  // plan_start
    std::array<double, 3>* ptr_s,            // 输出s
    std::array<double, 3>* ptr_d) {          // 输出d
...
Status LatticePlanner::PlanOnReferenceLine(
  ...
  auto ptr_path_time_graph = std::make_shared<PathTimeGraph>(//st图
      ptr_prediction_querier->GetObstacles(), *ptr_reference_line,
      reference_line_info, init_s[0],//规划的起点
      init_s[0] + FLAGS_speed_lon_decision_horizon, 0.0,//终点、开始的时间
      FLAGS_trajectory_time_length, init_d);//结束的时间

  double speed_limit =
      reference_line_info->reference_line().GetSpeedLimitFromS(init_s[0]);//得到速度的上限
  ...
  CollisionChecker collision_checker(frame->obstacles(), init_s[0], init_d[0],//将障碍物的轨迹给画出来一个一个的长方形
