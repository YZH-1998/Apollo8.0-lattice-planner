...
void PiecewiseAccelerationTrajectory1d::AppendSegment(const double a,
  ...
  double v1 = v0 + a * t_duration;//t_duration之后的速度
  ACHECK(v1 >= -FLAGS_numerical_epsilon);// > 0

  double delta_s = (v0 + v1) * t_duration * 0.5;//t_duration往前走的距离
  double s1 = s0 + delta_s;
  double t1 = t0 + t_duration;

  ACHECK(s1 >= s0 - FLAGS_numerical_epsilon);
  s1 = std::max(s1, s0);//s,v,t的状态采取加速a经过t_duration 可以到达s1,v1,t1
  s_.push_back(s1);
  v_.push_back(v1);
  a_.push_back(a);
  t_.push_back(t1);
...
double PiecewiseAccelerationTrajectory1d::Evaluate_v(const double t) const {
  auto it_lower = std::lower_bound(t_.begin(), t_.end(), t);//
  auto index = std::distance(t_.begin(), it_lower);//找到it_lower对应的最近的索引值
  ...
  double v = apollo::common::math::lerp(v0, t0, v1, t1, t);//还是采用前后两个点进行插值
  return v;
}
