...
SLBoundary PathTimeGraph::ComputeObstacleBoundary(
  ...
  for (const auto& point : vertices) {//把障碍物的全局坐标转化为frenet坐标系下
...
void PathTimeGraph::SetupObstacles(
  ...
  for (const Obstacle* obstacle : obstacles) {
    if (obstacle->IsVirtual()) {//不处理虚拟障碍物
      continue;
    }
    if (!obstacle->HasTrajectory()) {
      SetStaticObstacle(obstacle, discretized_ref_points);//障碍物没有轨迹设置为静态障碍物
    } else {
      SetDynamicObstacle(obstacle, discretized_ref_points);//设置为动态障碍物
    }
  }
...
void PathTimeGraph::SetStaticObstacle(
  ...
  SLBoundary sl_boundary =//边界计算完成s_min, s_max, l_min, l_max
  ...
  if (sl_boundary.start_s() > path_range_.second ||
    ...
    ADEBUG << "Obstacle [" << obstacle_id << "] is out of range.";//判断障碍物是否在规划的区域外
    return;
  }
  
  path_time_obstacle_map_[obstacle_id].set_id(obstacle_id);//设置点 没有别的操作
...
void PathTimeGraph::SetDynamicObstacle(
    const Obstacle* obstacle,
    const std::vector<PathPoint>& discretized_ref_points) {
  double relative_time = time_range_.first;//0
  while (relative_time < time_range_.second) {
    TrajectoryPoint point = obstacle->GetPointAtTime(relative_time);//relative_time时刻障碍物的各个信息
    Box2d box = obstacle->GetBoundingBox(point);//给一个时刻的点 画一次
    SLBoundary sl_boundary =
        ComputeObstacleBoundary(box.GetAllCorners(), discretized_ref_points);//仅仅获取box的四个角 然后投影
    ...
    if (sl_boundary.start_s() > path_range_.second ||
      ...
      if (path_time_obstacle_map_.find(obstacle->Id()) !=
          path_time_obstacle_map_.end()) {//障碍物消失了吧
        break;
      }
...
const std::vector<STBoundary>& PathTimeGraph::GetPathTimeObstacles() const {
  return path_time_obstacles_;//处理完动态和静态障碍物以后就传过来了
}
...
std::vector<std::pair<double, double>> PathTimeGraph::GetPathBlockingIntervals(
  ...
  for (const auto& pt_obstacle : path_time_obstacles_) {
    ...
    intervals.emplace_back(s_lower, s_upper);//t时刻对应的s坐标的上下界
  }
  return intervals;
}
...
std::vector<STPoint> PathTimeGraph::GetObstacleSurroundingPoints(
  ...
  size_t num_sections = static_cast<size_t>(time_gap / t_min_density + 1);//分的段数
  double t_interval = time_gap / static_cast<double>(num_sections);//每一段的时间间隔

  for (size_t i = 0; i <= num_sections; ++i) {
    double t = t_interval * static_cast<double>(i) + t0;
    double s = lerp(s0, t0, s1, t1, t) + s_dist;//根据时间对s坐标进行线性插值
