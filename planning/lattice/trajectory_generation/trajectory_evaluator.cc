...
TrajectoryEvaluator::TrajectoryEvaluator(
  ...
  path_time_intervals_ = path_time_graph_->GetPathBlockingIntervals(//s_upper, s_lower 获取有障碍物的时候的s
...
double TrajectoryEvaluator::LonObjectiveCost(
    const PtrTrajectory1d& lon_trajectory,
    const PlanningTarget& planning_target,
    const std::vector<double>& ref_s_dots) const {//ref_s_dots前面求出来的
  double t_max = lon_trajectory->ParamLength();
  double dist_s =
      lon_trajectory->Evaluate(0, t_max) - lon_trajectory->Evaluate(0, 0.0);//一共规划了多远吧 
  
  double speed_cost_sqr_sum = 0.0;
  double speed_cost_weight_sum = 0.0;
  for (size_t i = 0; i < ref_s_dots.size(); ++i) {//速度之间的误差
...
std::vector<double> TrajectoryEvaluator::ComputeLongitudinalGuideVelocity(
  ...
  if (!planning_target.has_stop_point()) {//没有停止点
    ...
    lon_traj.AppendSegment(//添加FLAGS_trajectory_time_length最后的状态s v t a 
        0.0, FLAGS_trajectory_time_length + FLAGS_numerical_epsilon);//规划最后时刻的速度 参数是加速度和时间间隔
    ...
    double a_comfort = FLAGS_longitudinal_acceleration_upper_bound * //舒适加速度，即车辆加速时允许的最大加速度
                       FLAGS_comfort_acceleration_factor;
    double d_comfort = -FLAGS_longitudinal_acceleration_lower_bound * //舒适减速度，即车辆减速时允许的最大减速度
                       FLAGS_comfort_acceleration_factor;
