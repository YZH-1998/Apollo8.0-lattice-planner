...
bool GridSearch::GenerateDpMap(
  ...
  while (!open_pq.empty()) {
    ...
    std::vector<std::shared_ptr<Node2d>> next_nodes = //eight nodes
        std::move(GenerateNextNodes(current_node));
    for (auto& next_node : next_nodes) {
      if (!CheckConstraints(next_node)) {//check if out of range or collision
        continue;
      }
      if (dp_map_.find(next_node->GetIndex()) != dp_map_.end()) {
        continue; // already exit in dp_map_ continue
      }
      if (open_set.find(next_node->GetIndex()) == open_set.end()) {//not in open_set_
        ++explored_node_num;// 
        next_node->SetPreNode(current_node);// set parent node
        open_set.emplace(next_node->GetIndex(), next_node);// not in open_set_: insert
        open_pq.emplace(next_node->GetIndex(), next_node->GetCost());// not in open_pq_: insert
      } else {//already in open_set_
        if (open_set[next_node->GetIndex()]->GetCost() > next_node->GetCost()) {//next_node->path_cost < the node already in open_set_
          open_set[next_node->GetIndex()]->SetCost(next_node->GetCost());// reverse cost
          open_set[next_node->GetIndex()]->SetPreNode(current_node);// revise parent node
        }
      }
    }
  }
...
double GridSearch::CheckDpMap(const double sx, const double sy) {
  ...
  if (dp_map_.find(index) != dp_map_.end()) {//in dp_map_
