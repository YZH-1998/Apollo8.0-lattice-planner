...
bool HybridAStar::ValidityCheck(std::shared_ptr<Node3d> node) {
  ...
  if (obstacles_linesegments_vec_.empty()) {//no obstacle no check return true
    return true;
  }
...
std::shared_ptr<Node3d> HybridAStar::Next_node_generator(
  ...
  if (next_node_index < static_cast<double>(next_node_num_) / 2) {//前一半是前进的
    steering =
        -max_steer_angle_ +
...
double HybridAStar::TrajCost(std::shared_ptr<Node3d> current_node,
  ...
  if (next_node->GetDirec()) {//forward penalty
  ...
  if (current_node->GetDirec() != next_node->GetDirec()) {//forward and backward change penalty
    piecewise_cost += traj_gear_switch_penalty_;
  }
  piecewise_cost += traj_steer_penalty_ * std::abs(next_node->GetSteer());//steer penalty
  piecewise_cost += traj_steer_change_penalty_ *
                    std::abs(next_node->GetSteer() - current_node->GetSteer());//steer chage penalty
...
bool HybridAStar::Plan(   
  ...
  for (const auto& obstacle_vertices : obstacles_vertices_vec) {//遍历障碍物列表
  ...
  if (!ValidityCheck(start_node_)) {//check start no collision
  ...
  if (!ValidityCheck(end_node_)) {//check end no collision
  ...
  while (!open_pq_.empty()) {
    ...
    for (size_t i = 0; i < next_node_num_; ++i) {
      ...
      if (open_set_.find(next_node->GetIndex()) == open_set_.end()) {// not in open_set_ 
        explored_node_num++;
